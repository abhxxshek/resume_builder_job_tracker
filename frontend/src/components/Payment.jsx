...
const Payment = () => {
  ...
  const paymentFetch = async (paymentId) => {
    try {
      ...
      console.log("Payment response:", res.data);
      // setResponseState(res.data);
      await savePayment(res.data,transactionId,selectedTemplate);
    } catch (error) {
      console.log("Error fetching payment:", error);
      toast.error("Payment verification failed!", { autoClose: 2000, position: "top-center" });
    }
  ...
  const savePayment = async (paymentData,transactionId,selectedTemplate) => {
    try {
      
      const res = await axiosInstance.post(`http://localhost:3000/payment/save-payment-details`, {paymentData,transactionId,selectedTemplate});
      if (!res.data.success) {
        toast.error(res.data.message, { autoClose: 2000, position: "top-center" });
        return;
      }
